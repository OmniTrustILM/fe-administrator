...
test.describe('AttributeFieldSelect', () => {
    ...
    const searchableCases = [
        {
            title: 'single-select renders a search box that filters options by label',
            overrides: { properties: { ...defaultProperties, multiSelect: false, label: 'CA Certificate' } },
        },
        {
            title: 'multi-select (resource content) renders a search box that filters options by label',
            overrides: {
    ...
    for (const { title, overrides } of searchableCases) {
        test(title, async ({ mount, page }) => {
            const descriptor = minimalDescriptor(overrides as any);
            const options = [
