...
export interface ClientCertificateRegistrationRequest {
    ...
    /**
     * Optional UUID of an existing certificate this registration succeeds. When set, the placeholder is linked to that certificate as its predecessor through a pending relation, classified at issuance as renewal, rekey or replacement. Completion runs through the standard issue flow, gated by this registration\'s own challenge when one was supplied; the source\'s own renew and rekey are unaffected. When omitted, the registration is a standalone placeholder completed by issue.
     * @type {string}
     * @memberof ClientCertificateRegistrationRequest
     */
    sourceCertificateUuid?: string;
    /**
     * Authorization secret (challenge) that gates completion of this pre-registered certificate. Write-only and optional — the operator supplies it to opt the registration into challenge-gated issuance; the platform never generates one. Challenge verification gates issue of this pre-registered certificate, and renew and rekey of it once issued.
     * @type {string}
     * @memberof ClientCertificateRegistrationRequest
     */
