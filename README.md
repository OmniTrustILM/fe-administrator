...
Because the tree cannot be regenerated, the headers under `src/types/openapi/utils` have been hand-edited to the `ILM Utils Service API` / `ilm@omnitrust.com` branding as an interim measure. These edits will be overwritten the next time regeneration works — the durable fix is the upstream `utils-service` rebrand, tracked under the platform rebrand epic [OmniTrustILM/ilm#108](https://github.com/OmniTrustILM/ilm/issues/108). Once that lands and the spec republishes, regeneration restores the same branding and the manual edit becomes moot.
